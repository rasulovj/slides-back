...
export class SlideGeneratorService {
    ...
    createTitleSlide(title, subtitle, imageUrl) {
    ...
    createStatsSlide(title, stats, imageUrl) {
        const slide = this.pptx.addSlide();
    ...
    createChartSlide(title, chartData, subtitle) {
        const slide = this.pptx.addSlide();
    ...
    createTimelineSlide(title, subtitle, steps) {
        const slide = this.pptx.addSlide();
        slide.background = { color: "F9FAFB" };
        // Purple accent corner
        slide.addShape(this.pptx.ShapeType.rect, {
    ...
    createClosingSlide(title, subtitle, contactInfo, imageUrl) {
        const slide = this.pptx.addSlide();
        // Purple background
    ...
    async generateFromContent(slides) {
        for (let i = 0; i < slides.length; i++) {
            const slideData = slides[i];
            switch (slideData.type) {
                case "title":
                    this.createTitleSlide(slideData.title, slideData.content[0], slideData.imageUrl);
                    break;
                case "stats":
                    if (slideData.stats) {
                        this.createStatsSlide(slideData.title, slideData.stats, slideData.imageUrl);
                    }
                    break;
                case "content":
                    if (slideData.chartData) {
                        this.createChartSlide(slideData.title, slideData.chartData, slideData.content[0]);
                    }
                    else {
                ...
                case "timeline":
                    ...
                    break;
                case "closing":
                    this.createClosingSlide(slideData.title, slideData.content[0], {
    ...
    createContentSlide(title, bulletPoints) {
        const slide = this.pptx.addSlide();
        slide.background = { color: "F9FAFB" };
        // Purple accent corner
        slide.addShape(this.pptx.ShapeType.rect, {
